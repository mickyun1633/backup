...
--	직원의 이름과 성은 이름이라는 별칭으로 연결해서 추력하되 급여 기준으로 오름차순 졍렬되도록
...
SELECT first_name || ' ' || lsat_name 이름, salary 급여, job_id 직무ID
FROM employees
WHERE salsry >= 5000 AND job_id = 'SA_MAN'
ORDER BY 급여;


SELECT first_name 이름, phone_number 전화번호, hire_data 입사일
FROM employees
