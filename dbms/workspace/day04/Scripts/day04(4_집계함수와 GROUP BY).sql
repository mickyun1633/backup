...
-- 전체 직원수 확인
SELECT COUNT(*) FROM HR.EMPLOYEES; -- 단일행 107

-- 특정 부서 110번의 직원수 확인
SELECT DISTINCT DEPARTMENT_ID
FROM HR.EMPLOYEES;

SELECT COUNT(*) AS "100번의 직원수"
FROM HR.EMPLOYEES 
...
-- 모든 직원의 급여 총합 계싼
SELECT SUM(SALARY)
FROM HR.EMPLOYEES; -- 1행

-- 부서별의 급여 총합 계산
SELECT DEPARTMENT_ID, SUM(SALARY) -- 오류 발생
...
FROM HR.EMPLOYEES
--WHERE AVG(SALARY) >= 5000	-- WHERE절에서는 집계함수 사용이 불가능하다
GROUP BY DEPARTMENT_ID;

SELECT DEPARTMENT_ID, AVG(SALARY)
FROM HR.EMPLOYEES
GROUP BY DEPARTMENT_ID
HAVING AVG(SALARY) >= 5000;	-- HAVING절에서 집계함수를 이용한 조건을 처리한다

-- ★★★★★
-- 작성순서 : SELECT -> FORM -> WHERE -> GROUP BY -> HAVING -> ORDER BY
-- 실행순서 : FORM -> WHERE -> GROUP BY -> HAVING -> SELECT -> ORDER BY

-- 부서들 급여 총합이 50000보다 작은 부서들만 내림차순으로 정렬
-- 1) 부서별 급여 총합 조회
...
-- 3) 각 학과별 평균 나이 조회
-- ROUND(컬럼값, 소수점몇자리까지) : 반올림함수
SELECT STU_DEPT 학과, ROUND(AVG(STU_AGE), 2) 평균나이 -- ROUND(컬럼값, 2) 소수점셋째자리에서반올림
FROM TBL_STU_G
GROUP BY STU_DEPT;
...
-- 6) 컴퓨터 공학과의 학생 중 25살이상인 학생 수 조회
