...
-- [1] 등가조인 : 두 개 이상의 테이블을 조인할 때 특정 컬럼들의 값이 서로 같은 행만 결과로 반환
...
SELECT COUNT(*)
FROM TBL_RENTAL TR; -- 5GOD

-- 정확히 같은 값을 가진 데이터로만 조회 1=1, 2=2,
SELECT TB.BOOKS_TITLE, TM.MEM_NAME, TM.MEM_ID, TB.BOOKS_ID 
...
SELECT  TB.BOOKS_TITLE,TM.MEM_NAME, TM.MEM_ID, TB.BOOKS_ID
FROM TBL_RENTAL tr 
JOIN TBL_MEMBERS TM ON TR.MEM_ID = TM.MEM_ID   -- 대여테이블 + 회원테이블 연결, 조건 : 빌린사람 번호 = 회원번호(같은사람끼리 연결)
JOIN TBL_BOOKS TB ON TR.BOOKS_ID = TB.BOOKS_ID; -- 대여테이블과 + 책 테이블 연결, 조건 : 빌린 책번호 = 책 번호(같은책끼리 연결)
-- 대여 테이블을 기준으로 해서 회원테이블을 회원번호와 연결하고
-- 책테이블과 책 번호를 연결해서 
-- 책 번호, 책 제목, 회원번호, 회원이름을 가져와라
...
--WHERE TR.REN_RENTAL_DATE  BETWEEN TO_DATE('2023-12-25', 'YYYY-MM-DD') AND TO_DATE('2023-12-25', 'YYYY-MM-DD');

-- [2] 비등가 조인 : 두 테이블 간의 조건이 같음이 아닌 범위 조건등을 사용하는 방식

-- 누가 어떤책을 얼마나 빌렸는지 보고 짧게 빌렸으면 단기대여 길면 장기대여로 조회(7일 이하면 단기, 7일 초과면 장기)
...
CREATE TABLE TBL_STU1(
   ...
   CONSTRAINT STU1_PK PRIMARY KEY(STU_SID)
);
...
-- 모든 학생 정보 기준으로 수강정보가 있는 경우 함께 출력하고 없는 경우 NULL 출력
SELECT STU_SID, STU_NAME, STU_MAJOR, ENR_SID, ENR_CID, ENR_GRADE
FROM TBL_STU1 S LEFT OUTER JOIN TBL_ENROLL E
ON S.STU_SID = E.ENR_SID;

-- [2] 오른쪽 외부조인 RIGHT OUTER JOIN
-- 수강 정보 기준으로 수강한 학생이 없는 경우에도 학생 정보는 NULL 출력 (표시)
SELECT * FROM TBL_ENROLL;
SELECT * FROM TBL_STU1;

SELECT STU_SID, STU_NAME, STU_MAJOR, ENR_SID, ENR_CID, ENR_GRADE
FROM TBL_STU1 S RIGHT JOIN TBL_ENROLL E
ON S.STU_SID = E.ENR_SID;

-- [3] 완전 외부조인 FULL OUTER JOIN
SELECT STU_SID, STU_NAME, STU_MAJOR, ENR_SID, ENR_CID, ENR_GRADE
FROM TBL_STU1 S FULL JOIN TBL_ENROLL E
ON S.STU_SID = E.ENR_SID;
...
-- 3. 교차조인(CROSS JOIN)
SELECT TB.*, BP.*
FROM TBL_BOOKS TB CROSS JOIN TBL_BOOKPRICE BP;

SELECT COUNT(*)
FROM TBL_BOOKS;

SELECT COUNT(*)
FROM TBL_BOOKPRICE tb;
