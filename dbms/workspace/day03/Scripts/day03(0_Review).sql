...
-- 총 보상 = (salary * 12) + (salary * commission_pct * 12)

-- NVL 함수
SELECT first_name || ' ' || last_name 이름,
   salary 급여, commission_pct 커미션, salary * 12 + (salary * NVL(commission_pct, 0) * 12) "총 보상"
FROM hr.employees;

-- NVL2 함수
SELECT first_name || ' ' || last_name 이름,
   salary 급여, commission_pct 커미션,
...
-- 테이블 : hr.employees
-- 조건1 : commission_pct is not null
...
-- 이메일 중복
--VALUES (3, '김철수', 'hong@test.com', 'M', 'Y', sysdate);
-- 성별 오류
--VALUES (4, '신짱구', 'gu@test.com', 'X', 'Y', sysdate);
-- 상태 오류
--VALUES (5, '신짱아', 'a@test.com', 'F', 'A', sysdate);